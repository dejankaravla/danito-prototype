...
function NavProducts({ navRightOpen, setPathTo, setPageName, setData }) {
  ...
  useEffect(() => {
    ...
    if (selected === PRODUCTS.prozori) {
      setButtons([
        PRODUCTS.pvcProzori,
        PRODUCTS.drveniProzori,
        PRODUCTS.drvoAluminiumProzori,
        PRODUCTS.aluminiumProzori,
      ]);
      setImg([IMAGES.windowPvcImg, IMAGES.windowWoodImg, IMAGES.windowWoodAluminiumImg, IMAGES.windowAluminiumImg]);
      setImgClass("ButtonNavLeftTopImg");
    }
    // Vrata
    if (selected === PRODUCTS.vrata) {
      setButtons([PRODUCTS.pvcVrata, PRODUCTS.aluminiumVrata]);
      setImg([IMAGES.doorPvcImg, IMAGES.doorAluminiumImg]);
      setImgClass("ButtonNavLeftTopImg");
    }
    // Roletne
    ...
    if (selected === PRODUCTS.pvcProzori) {
      setButtons([
        ...
        PRODUCTS.allProducts.pvcProzor3,
        PRODUCTS.allProducts.pvcProzor4,
        PRODUCTS.allProducts.pvcProzor5,
      ]);
      setImg("");
    }
    if (selected === PRODUCTS.drveniProzori) {
      setButtons([PRODUCTS.allProducts.drveniProzor1]);
      setImg("");
    }
    if (selected === PRODUCTS.drvoAluminiumProzori) {
      setButtons([PRODUCTS.allProducts.drvoAluminiumProzori]);
      setImg("");
    }
